...
typedef struct _aes_key_st {
  ALIGN(16) uint32_t k[AES_MAX_KEY_SIZE / 4];
  int bits;
} aes_key;

typedef struct _aes_ks_st {
  ALIGN(16) uint32_t rk[4 * (AES_MAX_ROUNDS + 1)];
  int nr;
} aes_ks;
...
#define aesni_ctr_encr(in, out, len, ks, ctr)                                  \
  aesni_ctr128_encr(in, out, len, ks, ivec, nonce)

#define aesni_ctr_decr(in, out, len, ks, ctr)                                  \
  aesni_ctr128_encr(in, out, len, ks, ctr)
